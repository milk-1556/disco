...
/**
 * A representative "template server" snapshot — branded for creator "Acme" with purple #7C3AED and a
 * Whop link. Shipped from core so tests, demos, and the MockGuild seeder can all share one canonical
 * sample of the kind of server Disco snapshots and rebrands.
 */
    ...
    channels: [
      ...
      { localRef: 'chan_roleselect', kind: 'text', name: 'role-select', categoryRef: 'cat_info', position: 3, copyPolicy: 'member_chat' },
      {
        ...
        overwrites: [
          ...
          { targetType: 'member', targetRef: 'member_222222222222222222', allow: '8', deny: '0' },
        ],
      },
...
/** A standard rebrand config: Acme → Nova, purple → rose, old Whop link → new. */
export function makeSampleConfig(): RebrandConfig {
